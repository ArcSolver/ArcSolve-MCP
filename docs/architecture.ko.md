...
<!-- i18n-source: architecture.md -->

ArcSolve-Kit은 **인기 서비스의 공개 API를 MCP 도구로 묶는** 모음이다.
...
(우선순위: 인자 > `ARCSOLVE_SERVICES` 환경변수 > 전체). CLI는 `arcsolve serve kakao`.
...
> `pyproject.toml`의 optional extras(`arcsolve[heavy]`)로 격리하고, **의존성 추가는 통합
...
자세한 절차는 [adding-a-skill.md](adding-a-skill.ko.md).
...
문서 배치 규칙은 [adding-a-service.md](adding-a-service.ko.md) 참고.
