...
> 작성 규칙은 [AGENTS.ko.md](../AGENTS.ko.md). 새 대상은 아래 **블록 템플릿**을 복사해 추가한다.

**상태 범례**: `planned`(대상 확정) · `in-progress`(구현 중) · `done`(검증 완료)

**포맷 태그**: 각 블록은 `format: mcp | skill | both`로 산출물 포맷을 명시한다(생략 시 `mcp`).
스킬 대상은 상류 API를 직접 치지 않고 **MCP 도구를 오케스트레이션**하므로(AGENTS.md 규칙 2-2),
도구 대신 **오케스트레이션할 MCP 도구(`allowed-tools`)**를 적는다.
