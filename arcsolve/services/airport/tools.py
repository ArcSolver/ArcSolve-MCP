...
def _hint_for_code(code: str, msg: str) -> str:
    """비-정상 결과코드를 canonical 안내로 해석한다(공유 _datagokr 위임).

    봉투에서 code/msg를 꺼내는 것은 이 서비스가 책임지고(airport는 `response.header.resultCode` +
    보조 cmmMsgHeader), 코드표 해석만 공유한다. 호출부가 비-OK 코드만 넘기므로 항상 문자열을 반환.
    코드표는 canonical을 쓰되, 트래픽 초과(22)는 인천공항 특화 문구(개발계정 일 500건)를 덧붙인다.
    """
    hint = _datagokr.explain_result_code(code, msg) or ""
    if code == "22":
        hint += " (개발계정은 일 500건 제한 — 운영계정 전환 시 상향 가능)"
    return hint
