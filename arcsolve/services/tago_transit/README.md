...
- 열차정보 TrainInfoService: https://www.data.go.kr/data/15098552/openapi.do
  - `getStrtpntAlocFndTrainInfo`(출/도착지기반 열차 — depPlaceId·arrPlaceId·depPlandTime)
...
| 열차 | `TrainInfoService/getStrtpntAlocFndTrainInfo?depPlaceId=&arrPlaceId=&depPlandTime=&…` |
...
| `dep/arr_station_id`(역ID) | `TrainInfoService/getCtyAcctoTrainSttnList`(시/도별 기차역 목록조회 — `cityCode` 기반, 본 MVP 외). 역ID는 응답 `nodeid`(역코드) |
...
- **서비스 경로 접미사 `…Service`** — 6종 전부 확인. 버스 3종(`ArvlInfoInqireService`/`BusSttnInfoInqireService`/`BusRouteInfoInqireService`)은 data.go.kr 상세의 Service URL 라벨로 직접 확인. 고속/시외/열차(`ExpBusInfoService`/`SuburbsBusInfoService`/`TrainInfoService`)는 다수 외부 구현의 실호출 URL로 교차확인.
- **열차 오퍼레이션** — 출/도착지기반 열차 스케줄은 `getStrtpntAlocFndTrainInfo`(역ID·출발일). `getCtyAcctoTrainList`는 존재하지 않는 오퍼레이션이라 수정함. TrainInfoService의 실제 오퍼레이션은 `getCtyCodeList`·`getCtyAcctoTrainSttnList`·`getVhcleKndList`·`getStrtpntAlocFndTrainInfo` 4종.
- **응답 필드명** — 고속/시외(`gradeNm`/`charge`/`depPlandTime`/`arrPlandTime`/`depPlaceNm`/`arrPlaceNm`/`routeId`)·열차(`traingradename`/`trainno`/`depplandtime`/`arrplandtime`/`depplacename`/`arrplacename`/`adultcharge`)는 다수 외부 구현으로 교차확인. **남은 미확정**: 노선 경유정류소(`getRouteAcctoThrghSttnList`)의 `nodeord`/`updowncd`는 인라인 스키마 미확인(동 네임스페이스 표준 표기 채택). 모델은 `extra="ignore"`로 변형을 흡수한다.
