...
출처(공식 — data.go.kr · 전부 TAGO 통합 네임스페이스 1613000):
  ...
  - 열차정보 TrainInfoService: https://www.data.go.kr/data/15098552/openapi.do
      · getCtyAcctoTrainList(도시간 열차정보 조회)
"""
...
# TODO(provenance): 고속/시외/열차 서비스 경로 끝의 `Service` 접미사는 대체 공지·국토부 표기에
#   따른 것이다. data.go.kr 영문 상세 페이지의 "Service URL" 라벨에는 축약형(`ExpBusInfo`/
#   `TrainInfo`)이 표시되나, 이는 표시용 약어로 보이며 실 호출 경로는 `…Service`다(버스 3종은
#   getCtyCodeList 샘플 링크로 `…Service` 확인됨). 라이브 키가 없어 4종 경로의 직접 검증은 보류.
...
OP_TRAIN = "/getCtyAcctoTrainList"  # 도시간 열차정보 조회
...
class TerminalBus(BaseModel):
    ...
    TODO(provenance): 필드명 대소문자(depPlandTime vs depplandtime)는 상류가 오퍼레이션별로
      다를 수 있다. 두 표기를 모두 받도록 별칭 없이 소문자/카멜 둘 다 선언하지 않고, 상세 페이지가
      카멜표기(요청변수)와 동일하게 응답을 주는 전제로 카멜을 채택하되 extra="ignore"로 흡수한다.
    """
...
class Train(BaseModel):
    """도시간 열차 운행 항목(getCtyAcctoTrainList).
    ...
    TODO(provenance): 열차 응답 필드명은 상세 페이지가 첫 오퍼레이션만 렌더해 인라인 확인이
      어려웠다(동 네임스페이스 열차 응답의 표준 소문자 표기 채택). extra="ignore"로 흡수한다.
    """
