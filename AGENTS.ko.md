...
- OAuth: `arcsolve.oauth.OAuthClient`(authcode+PKCE+refresh+토큰저장). OAuth 쓰는 서비스는
  `__init__.py`에서 `SERVICE`에 `make_auth_client`를 넘기면 `arcsolve auth <name>`이 코어 수정 없이 동작.
...
uv run arcsolve catalog   # docs/services.md + docs/skills.md 재생성
uv run arcsolve changelog # CHANGELOG.md 합본
```
